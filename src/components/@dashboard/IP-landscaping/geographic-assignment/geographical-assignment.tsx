...
import {
  getGeographicDistributionApplicant,
...
export const GeographicalDistributionAssignment: FunctionComponent<Props> = ({ keywords }) => {
  ...
  const transformData = (data: IGeoAssignment[] | undefined) => {
    if (!data) {
      return [];
    }
    ...
    //   children: [],
    // };
    const result: { country: string; children: { country: string; count: number }[] }[] = [];
    ...
    const groupedData: GroupedData = data.reduce((acc: GroupedData, item) => {
      const firstLetter =
        item.country !== null && item.country !== "" ? item.country.charAt(0).toUpperCase() : "";
      const secondLetter =
        item.country !== null && item.country.length > 1
          ? item.country.charAt(1).toLowerCase()
          : "";
      const groupName = firstLetter + secondLetter;

      acc[groupName] = acc[groupName] || { country: groupName, children: [] };
      ...
      >
        <div>
          <div className="space-y-8">
            ...
            <div>
              <TreeMap data={Category_H} name={"H: Electricountry"} />
            </div>
            <div>

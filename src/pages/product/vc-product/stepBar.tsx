    ...
    <div className="flex flex-col items-start">
      {Array.from({ length: totalSteps }, (_, index) => (
        <div key={index} className="flex flex-col items-center relative">
          ...
          {hoveredStep === index && (
            <div
              className="absolute left-[110%] top-0 text-nowrap text-center w-auto max-w-xs text-[12px] px-[15px] py-1 bg-[#FFA300] text-white rounded-lg shadow-hoverbox z-10 leading-none"
            >
              {stepDescriptions[index]}
            </div>
          )}

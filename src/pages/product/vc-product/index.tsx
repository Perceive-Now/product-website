    ...
    async (query: string, answer: string, file?: File) => {
      ...
      try {
        if (answer) {
          ...
          // const queries = { id: newQueryIndex, query, answer: "" };
          ...
          if (fileResponse) {
            const res = await dispatch(sendQuery({ user_input: fileResponse, user_id: userId || "" })).unwrap();
            if(res){
              dispatch(setVSChats({  id: newQueryIndex+1,
                query:
                  ...
                  <div className="">
                    {chats.map((chat, idx) => (
                      <>
                        ...
                        {chat.extract && <ExtractInfo info={chat.extract} />}
                      </>
                    ))}
                    {isLoading && (

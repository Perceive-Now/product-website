...
//
export function comparisonOfAssignmentConcentrationAmongOrganizations(
  data: PatentInfo[],
  ...
  const topStartYearConcentration =
    topOrgsStart.reduce((acc, curr) => acc + curr.count, 0) / totalStartYearPatents;
  const topEndYearConcentration =
    topOrgsEnd.reduce((acc, curr) => acc + curr.count, 0) / totalEndYearPatents;

  // Calculate the percentage change in concentration
  const concentrationChange =
    ((topEndYearConcentration - topStartYearConcentration) / topStartYearConcentration) * 100;

  return `The concentration of patent assignments among the top organizations has increased by ${concentrationChange.toFixed(
    2,
  )}% over the last ${numberOfYears} years, indicating a trend towards more centralized ownership of innovations.`;
}
